namespace Tests
{
    ...
    public static class TestDataProvider
    {
        ...
        {
            if (!context.FcmTokens.Any())
            {
                var bob = context.Users.Single(u => u.UserName == "Bob");
                var fcmToken1 = new FcmToken(
                    Guid.NewGuid().ToString(), bob);
                var fcmToken2 = new FcmToken(
                    Guid.NewGuid().ToString(), bob);

                context.FcmTokens.AddRange(
                    fcmToken1, fcmToken2);
                context.SaveChanges();
            }
        }
            ...
            if (!context.DeploymentPlans.Any())
            {
                    ...
                    "First test plan")
                {
                    DepartmentId = context.Departments
                        .Single(d => d.Name == "Administration").Id,
                };
                ...
                {
                    DepartmentId = context.Departments
                        .Single(d => d.Name == "Finance").Id,
                };
                var deploymentPlan3 = new DeploymentPlan(
                    DateTime.MinValue.AddDays(3),
                    DateTime.MaxValue.AddDays(-3),
                    Guid.NewGuid(),
                    ".exe",
                    "Third test plan")
                {
                ...
                context.DeploymentPlans.AddRange(
                    deploymentPlan1, deploymentPlan2, deploymentPlan3);
                context.SaveChanges();
            }
        }
    }
